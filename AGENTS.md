# AGENTS.md - AI Agent Development Guide for tap-yandex-cloud
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from YandexCloud.

### Key Components

1. **Tap Class** (`tap-yandex-cloud/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap-yandex-cloud/client.py`): Handles API communication and authentication
1. **Streams** (`tap-yandex-cloud/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap-yandex-cloud/streams.py`
...
```python
class MyNewStream(YandexCloudStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(YandexCloudStream):
...
```bash
tap-yandex-cloud --config config.json --discover
tap-yandex-cloud --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap-yandex-cloud/tap.py`
...
```bash
# .env.example
TAP_YANDEXCLOUD_API_URL=https://api.example.com
TAP_YANDEXCLOUD_API_KEY=your_api_key_here
TAP_YANDEXCLOUD_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-yandex-cloud --version`), Python version, and a minimal reproduction case when reporting bugs.
